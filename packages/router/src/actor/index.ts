...
export interface ActorBehavior<S> {
	...
	) => Promise<readonly [S, readonly ActorEffect[]]>;
}
...
export class ServiceTaskActor {
	static behavior(
		processId: string,
		taskId: string,
	): ActorBehavior<ServiceTaskState> {
		return {
			initialState: {
				processId,
				taskId,
							...
							[
								{
									...
									message: {
										id: "1",
										type: "task_completed",
										payload: { processId, taskId, result: message.payload },
									},
								},
							],
						];
								...
								[
									{
										...
										message: {
											id: "1",
											type: "task_failed_final",
											payload: { processId, taskId, error: message.payload },
										},
									},
								],
							];
		...
		return {
			...
			receive: async (state, message) => {
				switch (message.type) {
					...
					case "shutdown":
						return [{ ...state, status: "terminated" }, [{ type: "stop" }]];
	...
	spawn<S>(behavior: ActorBehavior<S>, name: string): Promise<ActorRef> {
		...
		const processMailbox = async (): Promise<void> => {
			while (mailbox.length > 0) {
				...
				const [newState, effects] = await behavior.receive(
					currentState,
					message,
				);
...
export class BPMNActorBridge {
	...
	async executeServiceTask(
		...
		try {
			...
			const taskActor = await this.system.spawn(
				ServiceTaskActor.behavior(processId, taskId),
				`task-${taskId}`,
			);
	...
	async startProcess(processId: string): Promise<void> {
		const supervisor = await this.system.spawn(
			ProcessSupervisorActor.behavior(processId),
...
// Factory functions
export const createActorSystem = (): EffectActorSystem =>
	new EffectActorSystem();

export const createBPMNBridge = (system: EffectActorSystem): BPMNActorBridge =>
	new BPMNActorBridge(system);
