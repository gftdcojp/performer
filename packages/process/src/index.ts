...
export class ProcessEngine {
  ...
  async signalProcess(instanceId: string, signalName: string, variables?: Record<string, any>): Promise<void> {
  ...
  async messageProcess(instanceId: string, messageName: string, variables?: Record<string, any>): Promise<void> {
    const instance = this.instances.get(instanceId)
    if (!instance) throw new Error(`Process instance ${instanceId} not found`)
  ...
  async completeTask(instanceId: string, taskId: string, variables?: Record<string, any>): Promise<void> {
    const instance = this.instances.get(instanceId)
    if (!instance) throw new Error(`Process instance ${instanceId} not found`)
...
// DSL helper
export function flow(id: string, name: string, builder: (p: ProcessBuilder) => ProcessBuilder): ProcessDefinition {
