...
export default function OrderPage({
	...
	const handleApproveOrder = async (approved: boolean, comments = "") => {
		setLoading(true);
		setMessage("");
	...
	const handleProcessPayment = async (paymentMethod: string) => {
		...
		try {
			...
			if (mockResult.success && mockResult.data) {
				setMessage(
					`Payment successful! Transaction ID: ${mockResult.data.transactionId}`,
				);
				await loadProcessStatus();
			} else {
			...
			<div className="min-h-screen bg-gray-50">
				...
				<div className="bg-white shadow rounded-lg p-6">
					...
					<div className="space-y-4">
						...
						{/* Manager Approval */}
						{processStatus?.tasks.some(
							(t: any) =>
								t.name === "Manager Approval" && t.status === "running",
						) && (
							<div className="space-y-2">
								<textarea
						...
						{/* Payment Processing */}
						{processStatus?.tasks.some(
							(t: any) => t.name === "Payment" && t.status === "running",
						) && (
							<div className="space-y-2">
