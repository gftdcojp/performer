...
export default function ProcessInstancesPage() {
  ...
  const applyFilters = () => {
    ...
    if (filters.processId) {
      filtered = filtered.filter(p => p.processId.includes(filters.processId));
    }
      ...
      <div className="max-w-7xl mx-auto py-8 px-4">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 mt-1">Monitor and manage running process instances</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold">Process Instances ({filteredProcesses.length})</h2>
          </div>
          ...
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div className="relative top-20 mx-auto p-5 border w-11/12 max-w-4xl shadow-lg rounded-md bg-white">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">Process Details: {selectedProcess.businessKey}</h3>
                <button
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="font-medium">ID:</span> {selectedProcess.id}</p>
                    <p><span className="font-medium">Process ID:</span> {selectedProcess.processId}</p>
                    <p><span className="font-medium">Status:</span> <span className={`px-2 py-1 rounded text-xs ${getStatusColor(selectedProcess.status)}`}>{selectedProcess.status}</span></p>
                    <p><span className="font-medium">Priority:</span> <span className={`px-2 py-1 rounded text-xs ${getPriorityColor(selectedProcess.priority)}`}>{selectedProcess.priority}</span></p>
                    <p><span className="font-medium">Started:</span> {selectedProcess.startTime.toLocaleString()}</p>
                    {selectedProcess.endTime && (
                      <p><span className="font-medium">Ended:</span> {selectedProcess.endTime.toLocaleString()}</p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="font-medium">Current Task:</span> {selectedProcess.currentTask || 'None'}</p>
                    <p><span className="font-medium">Assignee:</span> {selectedProcess.assignee || 'Unassigned'}</p>
                    <p><span className="font-medium">Duration:</span> {selectedProcess.duration || 'N/A'}</p>
                  </div>
