...
export const startOrder = actions.action(
  async (input: StartOrderInput, context): Promise<ActionResponse> => {
    ...
    try {
      ...
      throw new Error(`Failed to start order process: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
);
...
export const approveOrder = actions.action(
  async (input: ApproveOrderInput, context): Promise<ActionResponse> => {
    ...
    try {
      ...
      const instances = Object.values(processEngine).filter((inst: any) =>
        inst && typeof inst === 'object' && 'businessKey' in inst && inst.businessKey === businessKey
      );
...
export const getProcessStatus = actions.loader(
  async (context) => {
