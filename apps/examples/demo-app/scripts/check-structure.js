...
function checkStructure() {
	...
	if (hasSrc && hasApp) {
		...
		errors.push(
			"   This creates confusion between Vite (src/) and Next.js (app/) structures.",
		);
	...
	if (fs.existsSync(processesDir)) {
		...
		if (processFiles.length === 0) {
			errors.push(
				"⚠️  WARNING: No process definition files found in src/processes/",
			);
		}
	}
	...
	if (fs.existsSync(serverDir)) {
		...
		for (const serverSubDir of serverDirs) {
			...
			if (!hasActions && !hasLoader) {
				errors.push(
					`⚠️  WARNING: Server directory "${serverSubDir}" has no actions or loaders`,
				);
			}
		}
	}
	...
	if (errors.length === 0) {
		...
		console.log(
			`   - app/: ${hasApp ? "❌ (should be removed)" : "✅ (not present)"}`,
		);
		console.log(
			`   - src/components/: ${fs.existsSync(path.join(projectRoot, "src/components")) ? "✅" : "❌"}`,
		);
		console.log(
			`   - src/pages/: ${fs.existsSync(path.join(projectRoot, "src/pages")) ? "✅" : "❌"}`,
		);
		console.log(
			`   - src/processes/: ${fs.existsSync(path.join(projectRoot, "src/processes")) ? "✅" : "❌"}`,
		);
		console.log(
			`   - src/server/: ${fs.existsSync(path.join(projectRoot, "src/server")) ? "✅" : "❌"}`,
		);
		console.log("");
