...
describe("Business Actor Implementation", () => {
  describe("ActorFactory", () => {
    test("should create business actor factory", () => {
      expect(ActorFactory).toBeDefined()
      expect(typeof ActorFactory.createBusinessActor).toBe("function")
    })

    test("should create business actor with default config", async () => {
      const program = ActorFactory.createBusinessActor("test-actor")
    ...
    test("should create business actor with custom config", async () => {
      ...
      const program = ActorFactory.createBusinessActor("test-actor-config", config)

      const result = await Effect.runPromise(program)

      expect(result).toBeDefined()
      expect(result.id).toBe("test-actor-config")
    })
  })
